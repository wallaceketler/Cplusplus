...
decltype(auto) entrada(std::map<std::string, std::vector<std::pair<double, double>>> &roteiros){

    return roteiros.begin()->first;
}
...
int main() {
    ...
    auto x = entrada(roteiros);
    std::cout<<"Chamada função"<<std::endl;
    std::cout<<x<<std::endl;
